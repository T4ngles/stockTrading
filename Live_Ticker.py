...
Stock_Trace('NHC.AX', 6.18, False, datetime.date(2023, 10, 23), 0.30)
Stock_Trace('BOQ.AX', 5.35, False)
Stock_Trace('ASG.AX', 2.30, False)
Stock_Trace('CVL.AX', 0.88, False)
...
Stock_Trace('WLE.AX', 1.47, True, datetime.date(2023, 10, 17), 0.045)
Stock_Trace('ANZ.AX', 27.06, True)
Stock_Trace('WBC.AX', 23.05, True)

print(Stock_Trace._stock_dict)

for key, value in Stock_Trace._stock_dict.items():
...
in_money = ['FMG.AX', 'TLS.AX', 'ASH.AX', 'TCL.AX', 'ANZ.AX', 'WBC.AX', 'WLE.AX']
            ...
            if normalised_price > low_cutoff_ratio and normalised_price < high_cutoff_ratio:
                ...
                if Stock_Trace._stock_dict[stock_symbol].in_money:
                    plt.plot(range(normalized_data.index.size),
                             ...
                             label=f"{stock_symbol} (buy:${Stock_Trace._stock_dict[stock_symbol].price:.2f})(cur:${historical_data[stock_symbol][-i]:.2f})",
                             marker='x')
                else:
                    plt.plot(range(normalized_data.index.size),
                             normalized_data[stock_symbol],
